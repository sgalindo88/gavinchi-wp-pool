// ---------------------------------------------------------------------------
// Fill these in after deploying the Apps Script web app (see docs/setup.md).
// ---------------------------------------------------------------------------

// The Web App /exec URL from Deploy ▸ Manage deployments.
export const API_URL = 'PASTE_YOUR_APPS_SCRIPT_EXEC_URL_HERE';

// Folder (on this same GitHub Pages site) that serves the seed JSON files.
// Used only by the one-time "Seed sheet" admin action. Example:
//   https://yourname.github.io/wc/docs
export const DATA_BASE_URL = 'PASTE_YOUR_GITHUB_PAGES_DOCS_URL_HERE';

export const NUM_PLAYERS = 5;
export const TEAMS_PER_PLAYER = 9;
export const POLL_MS = 5000;
